...
async function añadirUsuario(){
    let datos = {};
    datos.nombre = document.getElementsByTagName('input')[0].value;
    datos.password = document.getElementsByTagName('input')[2].value
    datos.rol = 1;
    datos.email = null;

    const request = await fetch('/añadirUsuario', {
        method: 'POST',
        headers: {
